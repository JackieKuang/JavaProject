...
//http://estock.marbo.com.tw/html/stock_info_3130.asp
public class HTMLParserStock3 {
	...
	public void execute() throws Exception {
		...
		//String s1 = "3130,2494,2496,2506";
		
		String[] stockNum = s1.split(",");
		String dataTitle = "代碼,日期,開盤,最高,最低,收盤,漲跌,成交量,筆數,股本,預估EPS,收盤本益比,2008年 預估稅後盈餘,年度,股本(億),營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),資本公積(億),年度,股本(億),營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),資本公積(億),年度,股本(億),營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),資本公積(億),年度,股本(億),營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),資本公積(億),年度,股本(億),營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),資本公積(億),年度,營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),年度,營收(億),稅後淨利(億),EPS(元),毛利率(%),營益率(%),純益率(%),流動比率(%),負債比率(%),每股淨值(元),日期,融資張數,融資使用率,融券張數,融券使用率,外資張數,外資持有比率,日期,融資張數,融資使用率,融券張數,融券使用率,外資張數,外資持有比率,日期,融資張數,融資使用率,融券張數,融券使用率,外資張數,外資持有比率,日期,融資張數,融資使用率,融券張數,融券使用率,外資張數,外資持有比率,日期,融資張數,融資使用率,融券張數,融券使用率,外資張數,外資持有比率";
		
		
		WriteFile wf = new WriteFile("c:\\stock3\\分析資料\\","個股財務資料.csv");
		wf.writeFile(dataTitle);
		
		WriteFile wfLog = new WriteFile("c:\\stock3\\logs\\","HTMLParserStock3_log.txt");
		...
		for(int i=0; i<stockNum.length; i++) {
			try {
				...
				//System.out.println("http://stock.wearn.com/Financial.asp?mode=search&kind="+stockNum[i]);
		        ...
		        // Write data
				wf.writeFile(data1+data2+data3+data4+data5);
				wf.closeFile();
