...
/*
 * (3)Update ROE 負債比率 流動比率 速動比率
 * 財務比率（年表） 93 ~ 98
 * http://www.emega.com.tw/z/zc/zcr/zcra/zcra_1530.djhtm
 */
public class ROEYear {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			for(int p=0; p<stockNum.length; p++) {
				...
				String s = reader.readTextFile("http://www.emega.com.tw/z/zc/zcr/zcra/zcra_"+stockNum[p]+".djhtm");
				...
				String s1 = "";
				int[] yearArr = new int[1];
				String[] balanceArr = new String[1];
				for(int i=98, j=0; i>=98; i--,j++) {
...
//					System.err.println(yearArr[i]+":"+balanceArr[i]);
//				}
				
				
				//Update Data
				query = "update balance_year set roe=? where stock_id=? and year=?";
				stmt = conn.prepareStatement(query);
				
				for(int i=0; i<yearArr.length; i++) {
					try{
						stmt.setString(1,balanceArr[i]);
						stmt.setString(2,stockNum[p]);
						stmt.setInt(3,yearArr[i]);
						stmt.executeUpdate();
					}
					catch(Exception ex1) {
				...
				//負債比率
				len1 = s.indexOf("<td class=\"t4t1\">負債比率</td>");
				len2 = "<td class=\"t3n1\">".length();
				len3 = "<td class=\"t4t1\">負債比率</td>".length();
				len_start = len1+len2+len3;
				len_end = s.indexOf("</td>",len_start);
				s1 = "";
				yearArr = new int[1];
				balanceArr = new String[1];
				for(int i=98, j=0; i>=98; i--,j++) {
					s1 = s.substring(len_start,len_end);
					s1 = s1.replaceAll("\">","");
					s1 = s1.replaceAll("\"t3n1","");
...
//				}
				...
				//Update Data
				query = "update balance_year set debt_ratio=? where stock_id=? and year=?";
				stmt = conn.prepareStatement(query);
				...
				//流動比率
				len1 = s.indexOf("<td class=\"t4t1\">流動比率</td>");
				len2 = "<td class=\"t3n1\">".length();
				len3 = "<td class=\"t4t1\">流動比率</td>".length();
				len_start = len1+len2+len3;
				len_end = s.indexOf("</td>",len_start);
				s1 = "";
				yearArr = new int[1];
...
//				}
				...
				//Update Data
				query = "update balance_year set liquidity_ratio=? where stock_id=? and year=?";
				stmt = conn.prepareStatement(query);
				...
				//速動比率
				len1 = s.indexOf("<td class=\"t4t1\">速動比率</td>");
				len2 = "<td class=\"t3n1\">".length();
				len3 = "<td class=\"t4t1\">速動比率</td>".length();
				len_start = len1+len2+len3;
				len_end = s.indexOf("</td>",len_start);
				s1 = "";
				yearArr = new int[1];
...
//				}
				...
				//Update Data
				query = "update balance_year set quick_ratio=? where stock_id=? and year=?";
				stmt = conn.prepareStatement(query);
