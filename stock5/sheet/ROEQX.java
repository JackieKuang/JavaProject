...
public class ROEQX {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			for(int p=0; p<stockNum.length; p++) {
				ReaderURL reader = new ReaderURL();
				String s = reader.readTextFile("http://www.emega.com.tw/z/zc/zcr/zcr_"+stockNum[p]+".djhtm");
				
				//查無此股資料
				if(s.indexOf("查無")!=-1 || s.indexOf("個股代碼錯誤")!=-1) {
					continue;
				}
				
				//股東權益報酬率
				int len1 = s.indexOf("<td class=\"t4t1\">股東權益報酬率</td>");
				int len2 = "<td class=\"t3n1\">".length();
				int len3 = "<td class=\"t4t1\">股東權益報酬率</td>".length();
				int len_start = len1+len2+len3;
				int len_end = s.indexOf("</td>",len_start);
				String s1 = "";
				String[] balanceArr = new String[5];
				for(int j=0; j<balanceArr.length; j++) {
					s1 = s.substring(len_start,len_end);
					s1 = s1.replaceAll("\">","");
					s1 = s1.replaceAll("\"t3n1","");
					s1 = s1.replaceAll("\"t3r1","");
					s1 = s1.replaceAll("<p class=\"t3r1","");
					s1 = s1.replaceAll(",","");
...
//					System.err.println(balanceArr[i]);
//				}
				
				
				//Insert Data
				try{
					query = "insert into balance_qx(stock_id,quarter,year,roe,create_date) "+
							" values(?,3,99,?,now())";
					stmt = conn.prepareStatement(query);
					stmt.setString(1,stockNum[p]);
					stmt.setString(2,balanceArr[0]);
					stmt.executeUpdate();
				}
...
//				finally {if(stmt!=null) {stmt.close();}}
				...
				//每股淨值(元)
				len1 = s.indexOf("<td class=\"t4t1\">每股淨值(元)</td>");
				len2 = "<td class=\"t3n1\">".length();
				len3 = "<td class=\"t4t1\">每股淨值(元)</td>".length();
				len_start = len1+len2+len3;
				len_end = s.indexOf("</td>",len_start);
				s1 = "";
				balanceArr = new String[5];
				for(int j=0; j<balanceArr.length; j++) {
					s1 = s.substring(len_start,len_end);
					s1 = s1.replaceAll("\">","");
					s1 = s1.replaceAll("\"t3n1","");
...
//				}
				...
				//Update Data
				try{
					query = "update balance_qx set nav=? where stock_id=? and year=? and quarter=?";
					stmt = conn.prepareStatement(query);
					stmt.setString(1,balanceArr[0]);
					stmt.setString(2,stockNum[p]);
					stmt.setInt(3,99);
					stmt.setInt(4,3);
					stmt.executeUpdate();
				}
...
//				finally {if(stmt!=null) {stmt.close();}}
				...
				//每股稅後淨利(元)
				len1 = s.indexOf("<td class=\"t4t1\">每股稅後淨利(元)</td>");
				len2 = "<td class=\"t3n1\">".length();
				len3 = "<td class=\"t4t1\">每股稅後淨利(元)</td>".length();
				len_start = len1+len2+len3;
				len_end = s.indexOf("</td>",len_start);
				s1 = "";
				balanceArr = new String[5];
...
//				}
				...
				//Update Data
				try{
					query = "update balance_qx set tax_net_profit=? where stock_id=? and year=? and quarter=?";
					stmt = conn.prepareStatement(query);
					stmt.setString(1,balanceArr[0]);
					stmt.setString(2,stockNum[p]);
					stmt.setInt(3,99);
