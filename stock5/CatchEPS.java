...
//年報表資料 (http://stock.wearn.com/Financial.asp?mode=search&kind=3130) (聚財網)
//Table Name:stock_profit_year (一年跑一次,舊資料不必刪除)
public class CatchEPS {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0; i<stockNum.length; i++) {
			try {
				...
				String strAll = ru.readTextFile("http://stock.wearn.com/Financial.asp?mode=search&kind="+stockNum[i]);
		        ...
		        for(int j=0; j<tableTag.length; j++) {
		        	...
		        	if(j==table_num) {
		        		for(int k=0; k<tr_num.length; k++) {
		        			...
		        			try{
				        		...
				        		for(int j2=0; j2<td_Count; j2++) {
							        ...
							        for(int j3=0; j3<n1.length; j3++) {
							        	...
							        	if (n1[j3] instanceof TextNode) {
								        	...
								        	dataArr.add(textnode.getText().replaceAll(",",""));
								        }
							        }
						        ...
						        //System.out.println(tableTag[j].getRow(1).toHtml());	//tr
						        //System.out.println(tableTag[j].getRow(1).getChildCount());
						        //System.out.println(tableTag[j].getRow(1).getColumns()[0].getChildren().toHtml());	//td
		        			}
		        			catch(Exception ex) {}
			        			...
			        			try {
			        				...
			        				query = "insert into stock_profit_year(stock_id,year,gross,profitability,net_profit,fluid,speed,liability,interest,net_value,credit,equity,eps,create_date) values('"+stockNum[i]+"','"+(String)dataArr.get(0)+"','"+(String)dataArr.get(1).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(2).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(3).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(4).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(5).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(6).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(7).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(8).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(9).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(10).toString().replaceAll("&nbsp;","").replaceAll("%","")+"','"+(String)dataArr.get(11).toString().replaceAll("&nbsp;","").replaceAll("%","")+"',now());";
		        	...
		        	//System.out.println(tableTag[j].toHtml());
		        	//System.out.println("===================");
		        }
				
		        System.out.print(stockNum[i]+":"+i+"/"+stockNum.length+"\r");
			}
			catch(Exception ex) {
				conn.close();
