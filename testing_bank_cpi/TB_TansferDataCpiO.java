...
public class TB_TansferDataCpiO{
	//MYSQL Connection
	...
	public static void main(String[] args) throws Exception{
		...
		TB_TansferDataCpiO tbDao = new TB_TansferDataCpiO();
		tbDao.exeStart(args);
	...
	public void exeStart(String[] args) throws Exception{
		...
		for(int i=0;i<custno1.length;i++){
			wf=new WriteFile(logPath+"TB-TansferDataCpiO_"+custno1[i]+"-log.txt");
	...
	public void transferData(String custno1,String custno2) throws Exception{
	    ...
	    String sqlOracleCount="select count(*) NUM from B2B_VIP_USER u,B2B_VIP_ANSWER a,B2B_VIP_CUST_RESULT_SET r where r.BVCRS_ID in(select max(BVCRS_ID) from B2B_VIP_CUST_RESULT_SET where CUSTNO="+custno1+" and BVU_ID in(select max(BVU_ID) from B2B_VIP_CUST_RESULT_SET where CUSTNO="+custno1+" group by ID) group by BVU_ID) and u.BVU_ID=a.BVU_ID and u.BVU_ID=r.BVU_ID";
	    //String sqlOracleCount="select count(*) NUM from B2B_VIP_USER u,B2B_VIP_ANSWER a,B2B_VIP_CUST_RESULT_SET r where r.BVCRS_ID in() and u.BVU_ID=a.BVU_ID and u.BVU_ID=r.BVU_ID";
	    ...
	    String sqlOracle1="select u.ID,u.NAME,u.SEX,u.AGE_YEAR,u.AGE_MONTH,u.AGE_DAY,u.EDUCATION,u.EDU_DEP_ID,u.EDU_DEP_NAME,u.STATUS,u.INDUSTRY_ID,u.INDUSTRY_NAME,u.POSITION_ID,u.POSITION_NAME,u.TITLE,u.EXPIRE,u.TOTAL_EXPIRE,u.CREATE_DATE LOGIN_DATE,u.EMAIL,u.IDNO,a.*,r.CREATE_DATE EXAM_FINISHED_DATE,r.BVCRS_ID from B2B_VIP_USER u,B2B_VIP_ANSWER a,B2B_VIP_CUST_RESULT_SET r where r.BVCRS_ID in(select max(BVCRS_ID) from B2B_VIP_CUST_RESULT_SET where CUSTNO="+custno1+" and BVU_ID in(select max(BVU_ID) from B2B_VIP_CUST_RESULT_SET where CUSTNO="+custno1+" group by ID) group by BVU_ID) and u.BVU_ID=a.BVU_ID and u.BVU_ID=r.BVU_ID";
	    //String sqlOracle1="select u.ID,u.NAME,u.SEX,u.AGE_YEAR,u.AGE_MONTH,u.AGE_DAY,u.EDUCATION,u.EDU_DEP_ID,u.EDU_DEP_NAME,u.STATUS,u.INDUSTRY_ID,u.INDUSTRY_NAME,u.POSITION_ID,u.POSITION_NAME,u.TITLE,u.EXPIRE,u.TOTAL_EXPIRE,u.CREATE_DATE LOGIN_DATE,u.EMAIL,u.IDNO,a.*,r.CREATE_DATE EXAM_FINISHED_DATE,r.BVCRS_ID from B2B_VIP_USER u,B2B_VIP_ANSWER a,B2B_VIP_CUST_RESULT_SET r where r.BVCRS_ID in() and u.BVU_ID=a.BVU_ID and u.BVU_ID=r.BVU_ID";
	    ...
	    while(rsOracle1.next()){
			...
			try{
				...
				psMysqlInsert1.setString(2,rsOracle1.getString("ID"));
				psMysqlInsert1.setString(3,"0");
				psMysqlInsert1.setString(4,rsOracle1.getString("NAME"));
				...
				psMysqlInsert1.setString(10,rsOracle1.getString("STATUS"));
				...
				psMysqlInsert1.setString(15,null);
				psMysqlInsert1.setString(16,null);
				...
				psMysqlInsert1.setString(21,rsOracle1.getString("EMAIL"));
				psMysqlInsert1.setString(22,rsOracle1.getString("IDNO"));
				//insert TB_USER==============================END
				...
				psMysqlInsert2.setString(146,rsOracle1.getString("CREATE_DATE"));
				//insert TB_ANSWER_CPI==============================END
				
				//insert TB_RESULT_CPI==============================START
				EvalEachDimensionForeScoreO edfs=new EvalEachDimensionForeScoreO(connOracle);//計算職能PR值
				edfs.setDimensionScore(rsOracle1.getString("BVU_ID"),"1","1");
				int[] PR_C=edfs.getPR();//職能PR值
				EvalEachDimensionScoreO eds=new EvalEachDimensionScoreO();//計算性格PR值
				eds.setDimensionScore(connOracle,rsOracle1.getString("BVU_ID"),"1");
	    		...
	    		if(psMysqlInsert1.executeUpdate()==1 && psMysqlInsert2.executeUpdate()==1 && psMysqlInsert3.executeUpdate()==1 && psMysqlInsert4.executeUpdate()==1){
					connMysql.commit();
	    			insertCount++;
	        		System.err.println("OK B2B_VIP_USER.ID="+rsOracle1.getString("ID")+", totalCount="+totalCount+", rowCount="+rowCount+", insertCount="+insertCount);
	        	}
			}catch(Exception ex){
				connMysql.rollback();
	    		System.err.println("ERROR BVCRS_ID="+rsOracle1.getString("BVCRS_ID")+", ID="+rsOracle1.getString("ID")+", CUSTNO="+custno1+": "+ex.toString());
	    		wf.writeFile("ERROR BVCRS_ID="+rsOracle1.getString("BVCRS_ID")+", ID="+rsOracle1.getString("ID")+", CUSTNO="+custno1+": "+ex.toString()+"\n");
			}
	    }
