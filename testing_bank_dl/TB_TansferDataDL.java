...
public class TB_TansferDataDL {
	//CUSTNO
	String custno = GetProperty.readValue("custno");
	...
	public static void main(String[] args) throws Exception {
		...
		TB_TansferDataDL tbDao = new TB_TansferDataDL();
		tbDao.exeStart(args);
		
	}
	
	public void exeStart(String[] args) throws Exception {
		String tableList = "p_auo_exam_data,tb_auo_exam_data";
		String[] tableName = tableList.split(",");
		
		wf = new WriteFile(logPath + "TB-TansferDataDL_"+tableName[0]+"-log.txt");
		
		System.err.println("***** Start "+tableName[0]+" *****");
		transferData(tableName[0], tableName[1]);
		System.err.println("***** End "+tableName[0]+" *****");
		System.err.println("");
		System.err.println("All Finish!!");
	}
	
	
	public void transferData(String tName, String tNameNew) throws Exception {
		//MYSQL
	    ...
	    for(int i=0; i<columnTotal_Goal; i++) {
	        ...
	        if("PUED_ID".equals(columnName[i])) {
	        	columnName[i] = "AED_ID";
	        }
	    ...
	    String sqlMysqlInsert = "insert into "+tNameNew+"("+columnNameList+") values("+columnValuesList+")";
	    ...
	    while(rs1.next()) {
	    	...
	    	for(int i=1; i<=columnName.length; i++) {
	    		if("CUSTNO".equals(columnName[i-1])) {
	    			ps2.setString(i, custno);
	    		}
	    		else {
	    	...
	    	System.err.println("Table Name:"+tName+",totalCount:"+totalCount+"/"+rowCount+"/"+allCount);
	    }
		...
		System.err.println("Table Name:"+tName+",totalCount:"+totalCount+"/"+rowCount+"/"+allCount);
		wf.writeFile("Table Name:"+tName+",totalCount:"+totalCount+"/"+rowCount+"/"+allCount+"\n");
		wf.closeFile();
	}
}
