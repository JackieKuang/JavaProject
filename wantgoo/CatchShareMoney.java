...
//除權息資料(http://tw.stock.yahoo.com/d/s/company_6244.html) (Yahoo)
//http://tw.stock.yahoo.com/d/s/dividend_6244.html (Yahoo)
//Table Name:stock_share_money (每天跑一次,視情況而定,舊資料自動先刪除)
public class CatchShareMoney {
...
//		String s1 = "4430";
		...
		java.sql.PreparedStatement PS_INS = null, PS_UPD=null, PS1=null;
		java.sql.ResultSet RS1=null;
		
		//刪除資料
//		query = "delete from stock_share_money where year1="+year1;
//		stmt = conn.prepareStatement(query);
//		stmt.executeUpdate(query);
		
		//Query 當年資料
		String sqlStrQuery = "select stock_id from stock_share_money " +
				"where stock_id=? and year1=?";
		PS1 = conn.prepareStatement(sqlStrQuery);
		
		//Insert 當年資料
		String sqlStrIns = "insert into stock_share_money(stock_id,year1,date1,money1,date2,money2,money3,money_all,money_total,capital,create_date) "+
				" values(?,?,?,?,?,?,?,?,?,?,now())";
		PS_INS = conn.prepareStatement(sqlStrIns);
		
		//Update 當年資料
		String sqlStrUpd = "update stock_share_money set date1=?,money1=?,date2=?,money2=?,money3=?,money_all=?,money_total=?,capital=?,create_date=now() " +
				"where stock_id=? and year1=?";
		PS_UPD = conn.prepareStatement(sqlStrUpd);
		...
		for(int i=0; i<stockNum.length; i++) {
			try {
				//檢查是否有資料
				int dataFlag = 0; //0:無資料、1:有資料
				PS1.setString(1,stockNum[i]);
				PS1.setString(2,year1);
				...
				// Get URL Content (除權息日期)
				ReaderURL ru = new ReaderURL();
				String strAll = ru.readTextFile2("http://tw.stock.yahoo.com/d/s/company_"+stockNum[i]+".html","BIG5");
				
				String strSP = strAll.substring(strAll.indexOf("除權日期"),strAll.indexOf("最後過戶日"));
				
				String s = "<td width=\"150\" align=\"center\">";
				int len1 = strSP.indexOf(s);
				int len2 = s.length();
				int len_start = len1+len2;
				int len_end = strSP.indexOf("</td>",len_start);
				
				String date1 = strSP.substring(len_start,len_end);
				date1 = date1.replaceAll(" ","");
				String[] date1Arr = new String[3];
				if(date1.indexOf("-")==-1) {
					date1Arr = date1.split("/");
					date1 = date1Arr[1]+"-"+date1Arr[2];
				}
				
				
				s = "<td width=\"150\" align=\"center\">";
				len1 = strSP.indexOf(s,len_end);
				len2 = s.length();
				len_start = len1+len2;
				len_end = strSP.indexOf("</td>",len_start);
				
				String date2 = strSP.substring(len_start,len_end);
				date2 = date2.replaceAll(" ","");
				String[] date2Arr = new String[3];
				if(date2.indexOf("-")==-1) {
					date2Arr = date2.split("/");
					date2 = date2Arr[1]+"-"+date2Arr[2];
				}
				
				//System.err.println(date2Arr[0]);
				
				s = "<td colspan=\"3\" >";
				len1 = strAll.indexOf(s);
				len2 = s.length();
				len_start = len1+len2;
				len_end = strAll.indexOf("億",len_start);
				String capital = strAll.substring(len_start,len_end);
				...
				//Get URL Content (除權息資訊)
				ru = new ReaderURL();
				strAll = ru.readTextFile("http://tw.stock.yahoo.com/d/s/dividend_"+stockNum[i]+".html");
				strSP = strAll.substring(strAll.indexOf("<td align=\"center\" height=\"25\">"+year1+"</td>"),strAll.indexOf("<td align=\"center\" height=\"25\">"+(Integer.parseInt(year1)-1)+"</td>"));
				
				s = "<td align=\"center\">";
				len1 = strSP.indexOf(s);
				len2 = s.length();
				len_start = len1+len2;
				len_end = strSP.indexOf("</td>",len_start);
				
				String money1 = strSP.substring(len_start,len_end);
				...
				String money2 = strSP.substring(len_start,len_end);
				...
				String money3 = strSP.substring(len_start,len_end);
				money3 = money3.replaceAll(" ","");
				...
				String money_all = strSP.substring(len_start,len_end);
				money_all = money_all.replaceAll(" ","");
				...
				String money_total = strSP.substring(len_start,len_end);
				money_total = money_total.replaceAll(" ","");
...
//				System.err.println(money3);
//				System.err.println(money_all);
//				System.err.println(money_total);
//				System.err.println(capital);
				
				if(dataFlag==1) {	//1:有資料
					//Update Data
					PS_UPD.setString(1,date2);
					PS_UPD.setString(2,money1);
					PS_UPD.setString(3,date1);
					PS_UPD.setString(4,money2);
					PS_UPD.setString(5,money3);
					PS_UPD.setString(6,money_all);
					PS_UPD.setString(7,money_total);
					PS_UPD.setString(8,capital);
					PS_UPD.setString(9,stockNum[i]);
					PS_UPD.setString(10,year1);
					PS_UPD.executeUpdate();
				}
				else {
					//Insert Data
					PS_INS.setString(1,stockNum[i]);
					PS_INS.setString(2,year1);
					PS_INS.setString(3,date2);
					PS_INS.setString(4,money1);
					PS_INS.setString(5,date1);
					PS_INS.setString(6,money2);
					PS_INS.setString(7,money3);
					PS_INS.setString(8,money_all);
					PS_INS.setString(9,money_total);
					PS_INS.setString(10,capital);
